(function () {
    ...
    function startPlugin() {
        ...
        function ActorsListComponent(object) {
            ...
            // Замінюємо Lampa.Scroll на Lampa.Grid
            var grid = new Lampa.Grid({
                root: object.render(true),
                limit: 20
            });
            var items = [];
            var html = $('<div></div>');
            ...
            // Створюємо заголовок
            var head = new Lampa.Head(object.title);
            ...
            this.create = function() {
                ...
                // Додаємо сітку та заголовок до HTML-контейнера
                html.append(head.render());
                html.append(grid.render());
                
                // Налаштовуємо Grid для прокрутки та завантаження наступної сторінки
                grid.onScroll = function(type, position, total) {
                    if (type === 'down' && position >= total - 10 && !waitload && page < total_pages) {
                        self.next();
                    }
                };
            ...
            this.loadActors = function(pageNum) {
                var self = this;
                waitload = true; // Блокуємо повторне завантаження
            ...
            this.append = function(data) {
                ...
                data.results.forEach(function(element) {
                    // Використовуємо InteractionCategory для створення картки актора
                    ...
                    cards_to_add.push(card.render(true));
                    items.push(card);
                });
                    
                // Додаємо картки до Grid
                grid.append(cards_to_add);

                // Встановлюємо колекцію для навігації
                Lampa.Controller.collectionSet(grid.render());
                if (page === 1) {
                     Lampa.Controller.collectionFocus(items[active] ? items[active].render(true) : false, grid.render());
                }
            };
