(function () {  
	...
	// Функція для створення компонента налаштувань тільки при потребі  
	function createLogoSettings() {  
		if (!Lampa.SettingsApi.getComponent(LOGO_COMPONENT)) {  
	...
	Lampa.SettingsApi.addParam({  
		...
		onRender: function (item) {  
			item.on("hover:enter", function () {  
				createLogoSettings(); // Створюємо компонент тільки при потребі  
