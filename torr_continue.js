(function () {  
    ...
    // Реєстрація локалізації (самодостатній плагін)  
    Lampa.Lang.add({  
    ...
    function buildStreamUrl(params) {  
        ...
        if (!server_url) {  
            Lampa.Noty.show(Lampa.Lang.translate('continue_watch_torrserver_not_configured'));  
            return null;  
        }  
    ...
    function buildPlaylist(movie, currentParams, currentUrl, quietMode, callback) {  
        ...
        if (!quietMode) Lampa.Loading.start(function () { ABORT_CONTROLLER = true; finalize([]); }, Lampa.Lang.translate('continue_watch_preparing'));  
            ...
            var fetchFiles = function () {  
                ...
                Lampa.Torserver.files(torrent.hash, function (json) {  
                    ...
                    if (json && json.file_stats && json.file_stats.length > 0) {  
                        ...
                        retryCount++;  
                        if (!quietMode) Lampa.Loading.setText(Lampa.Lang.translate('continue_watch_waiting_files') + retryCount + '/' + maxRetries + ')...');  
    ...
    function launchPlayer(movie, params) {  
        ...
        if (isExternalPlayer) {  
            ...
            if (movie.number_of_seasons) tempPlaylist.push({ title: Lampa.Lang.translate('continue_watch_loading_playlist'), url: '', timeline: {} });  
            playerData.playlist = tempPlaylist;  
  
            if (timeline.time > 0) Lampa.Noty.show(Lampa.Lang.translate('continue_watch_restoring') + formatTime(timeline.time));  
            Lampa.Player.play(playerData);  
            ...
            if (movie.number_of_seasons && params.season && params.episode) {  
                buildPlaylist(movie, params, url, true, function (playlist) {  
                    if (playlist.length > 1) { Lampa.Player.playlist(playlist); Lampa.Noty.show(Lampa.Lang.translate('continue_watch_playlist_loaded') + playlist.length + ' эп.)'); }  
                });  
            }  
        }  
    }
        // ========================================================================  
    // 5. ЛОГИКА ПЛЕЕРА И ХУКИ (С ИСПРАВЛЕНИЕМ СИНХРОНИЗАЦИИ) — продовження  
    // ========================================================================  
    ...
    function handleContinueClick(movieData, buttonElement) {  
        ...
        if (!params) { Lampa.Noty.show(Lampa.Lang.translate('continue_watch_no_history')); return; }  
        ...
        Lampa.Listener.follow('full', function (e) {  
            if (e.type === 'complite') {  
                requestAnimationFrame(function () {  
                    ...
                    var labelText = Lampa.Lang.translate('continue_watch_continue');  
