// Об'єднаний плагін: ButtonSeparator + Optimized SVG Buttons
// Версія: 4.0 — Повна інтеграція двох плагінів

(function () {

    const PLUGIN_NAME = 'UnifiedButtons';
    let observer = null;

    /* ===========================
       1.  ВСТАНОВЛЕННЯ CSS ОДРАЗУ
    ============================== */
    ...
    /* ==========================================
       4. ОСНОВНІ ФУНКЦІЇ ПЕРШОГО ПЛАГІНА  
    =========================================== */
    ...
    function removeSourcesButton(container) {
        ...
        allButtons.each(function () {
            ...
            const isSources = text.includes('джерел') ||
            ...
            if (!important && (isPlay || isSources || (isOptions && isEmpty))) {
                button.remove();
            }
        });
    }
    ...
    function reorderButtons(box) {
        ...
        box.find('.full-start__button').each(function () {
            ...
            if (cls.includes('view--online')) order = 1;
            else if (cls.includes('view--torrent')) order = 2;
            else if (cls.includes('view--trailer')) order = 3;
