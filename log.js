(() => {
    const menuSelector = ".menu .menu__list .menu__item";
    const controlItems = [
        'Стрічка',
        'Фільми',
        'Серіали',
        'Мультфільми',
        'Особи',
        'Каталог',
        'Фільтр',
        'Релізи',
        'Вибране',
        'Історія',
        'Підписки',
        'Розклад',
        'Торренти',
        'Спорт'
    ];

    function initPlugin() {
        console.log("initPlugin викликано");
        if (window.plugin_hide_menu_ready) {
            console.log("plugin_hide_menu_ready вже true, вихід");
            return;
        }
        window.plugin_hide_menu_ready = true;

        if (!window.Lampa || !Lampa.SettingsApi || !Lampa.Listener || !Lampa.Storage) {
            console.warn("Lampa API не знайдено, плагін не може ініціалізуватися");
            return;
        }

        console.log("Додаємо компонент hide_menu");
        Lampa.SettingsApi.addComponent({
            component: "hide_menu",
            name: "Приховати пункти меню",
            icon: `<svg height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 4C7 4 2 7 2 12C2 17 7 20 12 20C17 20 22 17 22 12C22 7 17 4 12 4ZM12 18C8.14 18 5 14.86 5 11C5 7.14 8.14 4 12 4C15.86 4 19 7.14 19 11C19 14.86 15.86 18 12 18Z" fill="white"/>
                <circle cx="12" cy="11" r="3" fill="white"/>
            </svg>`
        });

        controlItems.forEach(title => {
            const paramName = `hide_menu_${title.toLowerCase().replace(/\s+/g, "_")}`;
            console.log("Додаємо параметр:", paramName);
            Lampa.SettingsApi.addParam({
                component: "hide_menu",
                param: {
                    name: paramName,
                    type: "select",
                    values: { "1": "Показати", "0": "Приховати" },
                    default: "1"
                },
                field: { name: title }
            });
        });

        Lampa.Listener.follow("settings", e => {
            console.log("Listener settings event:", e);
            if (e.component === "hide_menu" && e.type === "change") {
                console.log("Зміна в налаштуваннях hide_menu, оновлюємо меню");
                updateMenuVisibility();
            }
        });

        updateMenuVisibility();
    }

    function updateMenuVisibility() {
        console.log("updateMenuVisibility викликано");
        const menuItems = document.querySelectorAll(menuSelector);
        console.log("Знайдено пунктів меню:", menuItems.length);

        menuItems.forEach(item => {
            const textElem = item.querySelector(".menu__text");
            if (!textElem) {
                console.log("  — .menu__text не знайдено в пункті:", item);
                return;
            }
            const text = textElem.textContent.trim();
            console.log("  — пункт з текстом:", text);

            if (!controlItems.includes(text)) {
                console.log("    → пункт не в controlItems, пропускаємо");
                return;
            }

            const paramName = `hide_menu_${text.toLowerCase().replace(/\s+/g, "_")}`;
            const value = Lampa.Storage.get(paramName, "hide_menu");
            console.log("    paramName:", paramName, "value:", value);

            const show = (value === "0") ? false : true;
            console.log("    show:", show ? "показати" : "приховати");

            item.style.display = show ? "" : "none";
        });
    }

    if (window.appready) {
        initPlugin();
    } else {
        console.log("appready ще не готовий, чекаємо");
        if (window.Lampa && Lampa.Listener) {
            Lampa.Listener.follow("app", e => {
                console.log("Listener app event:", e);
                if (e.type === "ready") {
                    console.log("app ready — ініціалізація плагіну");
                    initPlugin();
                }
            });
        } else {
            window.addEventListener("load", () => {
                if (window.appready) {
                    console.log("Window load + appready, ініціалізація");
                    initPlugin();
                }
            });
        }
    }
})();
