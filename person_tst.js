(function() {
                ...
                (function (i) {
                    ...
                    new Lampa.Reguest().silent(url, function (response) {
                        try {
                            ...
                            if (json && json.id) {
                                // ВИПРАВЛЕННЯ: Зберігаємо картку з правильним типом для актора
                                var personCard = {
                                    id: json.id,
                                    title: json.name,
                                    name: json.name,
                                    poster_path: json.profile_path,
                                    // ВАЖЛИВО: Використовуємо правильний тип для актора
                                    type: "person", // Змінили з "actor" на "person"
                                    source: "tmdb",
                                    media_type: "person",
                                    // Додаємо додаткові поля для правильного відображення
    ...
    // Альтернативний спосіб через перехоплення подій
    function setupEventInterceptor() {
        document.addEventListener('click', function(e) {
            var activity = Lampa.Activity.active();
            if (!activity || activity.source !== PLUGIN_NAME) return;
            
            // Знаходимо картку, на яку клікнули
            var card = e.target.closest('.card');
            if (card) {
                var personId = card.getAttribute('data-id');
                var personName = card.getAttribute('data-name') || card.querySelector('.card__title')?.textContent;
                
                if (personId && personName) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    log('Intercepted click on person card:', personId, personName);
        ...
        }, true); // Використовуємо capture phase для перехоплення події
    }
