(function () {
    ...
    const PLUGIN_NAME = "persons";
    const PERSONS_KEY = "saved_persons";
    const PAGE_SIZE = 20;
    const DEFAULT_PERSON_IDS = [];
    ...
    function addButtonToContainer(bottomBlock) {
        const existingButton = bottomBlock.querySelector(".button--subscribe-plugin");
        ...
        const buttonsContainer = bottomBlock.querySelector(".full-start__buttons");
    ...
    // ==== СЕРВІС ДЛЯ ВИВОДУ ФІЛЬМОГРАФІЇ ====
        ...
        this.list = function (params, onComplete) {
            ...
            pageIds.forEach((personId) => {
                ...
                const url = Lampa.TMDB.api(
                    `person/${personId}/combined_credits?api_key=${Lampa.TMDB.key()}&language=${currentLang}`
                );
                        ...
                        try {
                            ...
                            if (json && json.cast) {
                                const credits = json.cast.map((item) => ({
                                    ...
                                    type: item.media_type, // "movie" або "tv"
                                    source: "tmdb",
                                    media_type: item.media_type,
                                }));

                                cache[personId] = credits;
    ...
    function startPlugin() {
        ...
        function checkCurrentActivity() {
            ...
            if (activity && activity.component === "actor") {
                let id = activity?.id || activity?.params?.id;
                if (!id && location.pathname.includes("/actor/")) {
                    id = location.pathname.match(/\/actor\/(\d+)/)?.[1];
                }
                currentPersonId = id ? parseInt(id, 10) : null;
                if (currentPersonId) {
