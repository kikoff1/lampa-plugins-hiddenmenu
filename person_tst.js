(function() {
    ...
    function hideSubscribeButton() {
        ...
        const css = `
            .card-subscribe, .button--subscribe {
    ...
    var pluginTranslations = {
        ...
        subscriibbe: { ru: "Подписаться", en: "Подписаться", uk: "Підписатися", be: "Падпісацца", pt: "Inscrever", zh: "订阅", he: "הירשם", cs: "Přihlásit se", bg: "Абонирай се" },
        unsubscriibbe: { ru: "Отписаться", en: "Отписаться", uk: "Відписатися", be: "Адпісацца", pt: "Cancelar inscrição", zh: "退订", he: "בטל מנוי", cs: "Odhlásit se", bg: "Отписване" },
    ...
    function addButtonToContainer(bottomBlock) {
        ...
        var buttonText = issubscriibbed ? Lampa.Lang.translate('persons_plugin_unsubscriibbe') : Lampa.Lang.translate('persons_plugin_subscriibbe');
        var button = $(`<div class="full-start__button selector button--subscriibbe-plugin ${issubscriibbed ? 'button--unsubscriibbe' : 'button--subscriibbe'}" data-focusable="true">
        ...
        button.on('hover:enter', function() {
            ...
            var newText = wasAdded ? Lampa.Lang.translate('persons_plugin_unsubscriibbe') : Lampa.Lang.translate('persons_plugin_subscriibbe');
            button.find('span').text(newText);
            button.toggleClass('button--unsubscriibbe', wasAdded);
            button.toggleClass('button--subscriibbe', !wasAdded);
            updatePersonsList();
        });

        var buttonsContainer = bottomBlock.querySelector('.full-start__buttons');
        if (buttonsContainer) buttonsContainer.append(button[0]);
    }
    ...
    function addsubscriibbeButton() {
        ...
        if (bottomBlock) {
            ...
            const interval = setInterval(function() {
                ...
                if (bottomBlock || attempts >= maxAttempts) {
    ...
    function PersonsService() {
        ...
        this.list = function(params, onComplete) {
            ...
            var personIds = getPersonIds().reverse(); // Показувати останніх доданих першими
            ...
            pageIds.forEach(function(personId) {
                if (cache[personId]) {
                ...
                new Lampa.Reguest().silent(url, function(response) {
                    try {
                        ...
                        if (json && json.id) {
                            ...
                            // Ми чітко вказуємо Lampa, що це картка типу "person"
                            // і що повну інформацію про неї треба брати з джерела "tmdb".
                            // Це дозволяє Lampa згенерувати правильне посилання на сторінку актора.
                            var personCard = {
                                id: json.id,
                                name: json.name,
            ...
            function checkComplete() {
                ...
                if (loaded >= pageIds.length) {
                    // Сортуємо результати відповідно до початкового порядку в pageIds
                    results.sort((a, b) => pageIds.indexOf(a.id) - pageIds.indexOf(b.id));
                    onComplete({ results: results, page: page, total_pages: Math.ceil(personIds.length / PAGE_SIZE), total_results: personIds.length });
                }
            }
        };
    }
    ...
    function startPlugin() {
        ...
        Lampa.Listener.follow('activity', function(e) {
            if (e.type === 'start' && e.object.component === 'actor') {
                currentPersonId = parseInt(e.object.id || 0, 10);
                if (currentPersonId) {
                    addsubscriibbeButton();
                }
            } else if (e.type === 'resume' && e.object.component === 'category_full' && e.object.source === PLUGIN_NAME) {
                 // Оновлюємо список, якщо повернулись на сторінку плагіна
                Lampa.Activity.reload();
            }
        });
        addButtonStyles();
