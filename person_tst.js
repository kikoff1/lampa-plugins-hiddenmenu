(function() {
    ...
    var pluginTranslations = {
        ...
        subscriibbe: { ru: "Подписаться", en: "subscriibbe", uk: "Підписатися", be: "Падпісацца", pt: "Inscrever", zh: "订阅", he: "הירשם", cs: "Přihlásit se", bg: "Абонирай се" },
        unsubscriibbe: { ru: "Отписаться", en: "Unsubscriibbe", uk: "Відписатися", be: "Адпісацца", pt: "Cancelar inscrição", zh: "退订", he: "בטל מנוי", cs: "Odhlásit se", bg: "Отписване" },
        ...
        this.list = function(params, onComplete) {
            ...
            for (var i = 0; i < pageIds.length; i++) {
                (function(i){
                    ...
                    if (cache[personId]) { results.push(cache[personId]); checkComplete(); return; }
            ...
            function checkComplete() {
                loaded++;
                if (loaded >= pageIds.length) onComplete({ results: results.filter(Boolean), page: page, total_pages: Math.ceil(personIds.length / PAGE_SIZE), total_results: personIds.length });
            }
        };
    }
    ...
    function startPlugin() {
        ...
        menuItem.on("hover:enter", function() {
            Lampa.Activity.push({ component: "category_full", source: PLUGIN_NAME, title: Lampa.Lang.translate('persons_plugin_title'), page: 1, url: PLUGIN_NAME + '__main' });
        });
        ...
        // Відкриття акторів при кліку на список
