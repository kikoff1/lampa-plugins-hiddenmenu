...
(function () {    
  ...
  function healthUrlCandidates(server) {    
    ...
    // Використовуємо ендпоінт тесту швидкості замість /config [1](#3-0)   
    return protos.map(function (p) { return p + url + '/download/300'; });    
  }    
    
  // HEALTH 3-статуси для TorrServer з логікою тесту швидкості    
  function runHealthChecks(servers) {    
    ...
    var requests = servers.map(function (server) {    
      return new Promise(function (resolve) {    
        ...
        // Збільшено таймаут для тесту швидкості [2](#3-1)   
        ajaxTryUrls(urls, 10000).then(function (res) {    
          var val;    
    
          if (res.ok) {    
            // Тест швидкості успішний - сервер доступний    
    ...
    /* =========================    
   * 5) Модалка (UI) - видалено кнопку перевірки підключення    
   * ========================= */    
  ...
  function torrserverSetting() {    
    ...
    Lampa.Settings.listener.follow('open', function (e) {    
      if (e.name === 'server') {    
        ...
        // Додаємо після основного посилання TorrServer [1](#4-0)   
