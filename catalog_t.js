...
(function () {      
  ...
  var serversInfo = [      
    {      
      ...
      settings: { url: '91.238.105.72:8090', login: 'stepan', password: 'bandera' }      
    },     
    {      
  ...
  function runHealthChecks(servers) {      
    ...
    var requests = servers.map(function (server) {      
      return new Promise(function (resolve) {      
        var healthUrls = healthUrlCandidates(server);      
        var healthCacheKey = 'health::' + server.base + '::' + healthUrls.join('|');      
        var cached = cache.get(healthCacheKey);      
  ...
  function applyMapToList(statusMap) {      
    window.currentModalList.find('.bat-torrserver-modal__item').each(function () {      
      var it = $(this);      
  ...
  function openTorrServerModal() {      
    ...
    window.currentModalList = list; // Зберігаємо посилання для оновлення статусів      
