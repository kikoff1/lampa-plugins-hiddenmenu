(function() {  
    ...
    Lampa.Lang.add({      
        ...
        location_redirect_constant: {  
            ru: 'Постоянный редирект',  
            uk: 'Постійний редірект',  
            en: 'Constant redirect'  
        },  
        location_redirect_constant_desc: {  
            ru: 'Чтобы отключить постоянный редирект нажмите клавишу ВНИЗ при загрузке приложения',  
            uk: 'Щоб відключити постійний редірект натисніть клавішу ВНИЗ при завантаженні додатку',  
            en: 'To disable constant redirect press DOWN key during app loading'  
        }  
    });       
    ...
    function startMe() {       
        ...
        // Додаємо параметр постійного редіректу  
        Lampa.SettingsApi.addParam({  
            component: 'location_redirect',  
            param: {  
                name: 'const_redirect',  
            ...
            field: {  
                name: Lampa.Lang.translate('location_redirect_constant'),  
                description: Lampa.Lang.translate('location_redirect_constant_desc')  
            }  
        });  
  
        // Слухач клавіш для відключення постійного редіректу  
        ...
        // Логіка постійного редіректу  
        setTimeout(function () {  
            if(Lampa.Storage.get('const_redirect') == true) {  
                window.location.href = server_protocol + Lampa.Storage.get('location_server');  
            }  
        }, 300);  
    }       
