// Версія плагіну: 4.2 - Виправлена версія  
...
(function() {      
    ...
    function processButtons(event) {  
        try {  
            ...
            let mainContainer = render.find('.full-start-new__buttons, .full-start__buttons, .buttons--container').first();  
            ...
            if (!hiddenContainer.length) {  
                console.warn(`${PLUGIN_NAME}: Прихований контейнер не знайдено`);  
                return;  
            }  
              
            // Переміщуємо кнопки з урахуванням налаштувань  
            const onlineBtn = hiddenContainer.find('.view--online');  
            const torrentBtn = hiddenContainer.find('.view--torrent');  
            const trailerBtn = hiddenContainer.find('.view--trailer');  
              
            if (onlineBtn.length > 0 && settings.enable_online) {  
            ...
            // Обробка кнопок після завершення  
            setTimeout(() => {  
    ...
    // === Виправлена функція збереження налаштувань ===  
    function saveButtonSettings() {  
        let settings = applySettings();  
        let newOrder = [];  
          
        // Використовуємо більш надійний метод ідентифікації  
        $('.button-edit-list__item').each(function() {  
            const $item = $(this);  
            const $title = $item.find('.button-edit-list__title');  
            const titleText = $title.text().trim();  
              
            // Знаходимо тип кнопки за допомогою data-атрибута або класу  
            let buttonKey = null;  
              
            // Перевіряємо data-атрибут якщо є  
            if ($item.data('button-key')) {  
                buttonKey = $item.data('button-key');  
            } else {  
                // Резервний метод - пошук за назвою  
                const buttonMap = {  
                    'Дивитись': 'play',  
                    'Онлайн': 'online',   
                    'Торрент': 'torrent',  
                ...
                buttonKey = buttonMap[titleText];  
            }  
              
            if (buttonKey) {  
                newOrder.push(buttonKey);  
                const isEnabled = $item.find('.dot').attr('opacity') == '1';  
                settings[`enable_${buttonKey}`] = isEnabled;  
                  
                console.log(`${PLUGIN_NAME}: Збережено кнопку ${buttonKey}:`, {  
                    enabled: isEnabled,  
        ...
        // Зберігаємо з перевіркою  
        try {  
            Lampa.Storage.set('unified_button_manager_settings', settings);  
              
            // Перевіряємо, чи налаштування дійсно збережено  
            const savedSettings = Lampa.Storage.get('unified_button_manager_settings');  
            if (!savedSettings || JSON.stringify(savedSettings) !== JSON.stringify(settings)) {  
                console.error(`${PLUGIN_NAME}: Помилка збереження налаштувань`);  
                return false;  
            }  
              
            console.log(`${PLUGIN_NAME}: Налаштування успішно збережено:`, settings);  
            ...
            console.error(`${PLUGIN_NAME}: Помилка при збереженні налаштувань:`, error);  
            return false;  
        }  
    }  
  
    // === Виправлена функція редагування кнопок ===  
    function editButtons() {  
        ...
        buttonTypes.forEach((buttonType) => {  
            ...
            let item_sort = $(`<div class="button-edit-list__item" data-button-key="${buttonType.key}">  
        ...
        Lampa.Modal.open({  
            ...
            onBack: () => {  
                const saved = saveButtonSettings();  
                if (saved) {  
                    Lampa.Noty.show('Налаштування збережено');  
                } else {  
                    Lampa.Noty.show('Помилка збереження налаштувань');  
                }  
                Lampa.Modal.close();  
    ...
    // === Покращена функція застосування налаштувань ===  
    function applyCurrentSettings() {  
        const settings = applySettings();  
          
        console.log(`${PLUGIN_NAME}: Застосування налаштувань:`, settings);  
        ...
        $('.full-start__button').each(function() {  
            ...
            if (shouldHide) {  
                button.hide();  
                console.log(`${PLUGIN_NAME}: Приховано кнопку:`, classes);  
            } else {  
                button.show();  
            }  
        });  
    ...
    function initPlugin() {  
        ...
        Lampa.Listener.follow('full', function(event) {  
            if (event.type === 'complete') { // ← Виправлено з 'complite' на 'complete'  
                setTimeout(() => {  
